---
title: "Kaggle Titanic Competition - Data Exploration & Model Building"
...
boostPred <- predict(object = boostFit, newdata = test.imp)
...
boostProb <- data.frame(boost=predict(object = boostFit, newdata = test.imp,type = 'prob')[[1]])
boostSigPred <- predict(object = boostsigmoidCal,newdata = boostProb,type = 'response')
boostSigPred <- ifelse(boostSigPred>=0.5,'Survived','Dead')

xgbsmotesigmoidCal
xgbProb <- data.frame(xgbsmote=predict(object = xgbsmoteFit, newdata = test.imp,type = 'prob')[[1]])
xgbsmoteSigPred <- predict(object = xgbsmotesigmoidCal,newdata = xgbProb,type = 'response')
xgbsmoteSigPred <- ifelse(xgbsmoteSigPred>0.5,'Survived','Dead')
```
