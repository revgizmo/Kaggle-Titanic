...
train.raw %>%
    ...
    arrange(-Median_Age) -> MedianAge

kable(MedianAge, format = 'html',align = 'c')
```
...
**Child?:** Trying out two engineered variables here - is the passenger a child or not? And is s/he close enough to be considered a adult by chance?
...
**Cabin related:** Extracting the cabin alphabet and number from the cabin variable. Also, some folks have more than 1 cabin. Wonder if that's important. Since lots of unknowns in the Cabin variable, all NA values are replaced by 'U'. Refering to the deck diagram, the topmost decks are A and B, which are closest to the lifeboats. Perhaps that's important too.
...
train.imp$CabinNum[is.na(train.imp$CabinNum)] <- median(train.imp$CabinNum,na.rm = T)
...
rfGrid <- data.frame(mtry=seq(8,12))
...
```{r}
re <- resamples(x = list(xgb=xgbFit,rf=rfFit.y))
summary(re)
```
...
age.predicted <- predict(rfFit, newdata = missing.age)
age.predicted[missing.age$title=='Master'] <- 4
...
test.imp$CabinNum[is.na(test.imp$CabinNum)] <- 40.5
...
test.imp$Fare[test.imp$Fare>159] <- 159
