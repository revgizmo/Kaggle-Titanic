...
# Misc - delete later...
```{r}
d <- data.frame(boostPred,xgbPred,rfPred,xgbsmotePred,rfsmotePred,boostSigPred)
map_df(d,~as.numeric(.x)*-1+2) -> d
d %>% mutate(Avg=rowMeans(.)) -> d
d %>% rownames_to_column('rowid') %>% reshape2::melt(id.vars='rowid') %>% 
...
```{r}
train.imp %>% 
    bind_cols(simulatedTrain) %>%
    mutate(Young=as.factor(Young)) %>% 
    group_by(Young) %>%
    summarize(
        A=mean(Age,na.rm = T),
        S=sd(Age)
