...
4. `SibSp` & `Parch`: What's interesting here is, for both these variables, at level 0, the fatality rate is higher. At levels 1+, the chances of survival are much better. Again, this points to the 'women *and children*' policy being followed.
...
```{r, message=FALSE, warning=FALSE}
ggplot(train.raw,aes(y=Age,x=Pclass))+geom_boxplot(aes(by=Survived,fill=Survived))+theme_bw()
...
ggplot(train.raw,aes(y=SibSp,x=Parch))+
    ...
    theme_bw()+
    scale_shape(solid=F)+
    geom_vline(xintercept = 3,color='gray',lty=3)+
    geom_hline(yintercept = 3,color='gray',lty=3)
```
...
train.imp$title[train.imp$title %in% c('Capt','Col')] <- 'Senior'
train.imp$title[train.imp$title %in% c('Don','Dr','Major','Mr','Rev','Sir','Jonkheer')] <- 'Mr'
train.imp$title[train.imp$title %in% c('Countess','Lady','Mrs','Mlle','Mme','Dona')] <- 'Mrs'
train.imp$title[train.imp$title %in% c('Ms')] <- 'Miss'
...
```{r}
age.predictors <- train.imp %>% dplyr::select(-Survived,-Cabin,-Ticket)
age.predictors <- age.predictors[complete.cases(age.predictors),]
...
rpartFit
plot(rpartFit)
# fancyRpartPlot(rpartFit$finalModel,sub = '',palettes = 'Blues')
```
...
```{r}
missing.age <- train.imp %>% filter(is.na(Age)) %>% dplyr::select(-Survived,-Cabin,-Ticket)
...
quantile(train.imp$Fare[train.imp$Pclass==1],probs = c(.1,.25,.5,.75,.90))
train.imp$Fare[train.imp$Fare>159] <- 159
```

# Modeling

```{r}
train.imp <- train.imp %>% dplyr::select(-Ticket,-Cabin)
response <- 'Survived'
...
xgb.importance(feature_names = colnames(Dtrain),model = xgbFit$finalModel) %>% xgb.plot.importance()
```
...
test.imp$title[test.imp$title %in% c('Capt','Col')] <- 'Senior'
test.imp$title[test.imp$title %in% c('Don','Dr','Major','Mr','Rev','Sir','Jonkheer')] <- 'Mr'
test.imp$title[test.imp$title %in% c('Countess','Lady','Mrs','Mlle','Mme','Dona')] <- 'Mrs'
test.imp$title[test.imp$title %in% c('Ms')] <- 'Miss'
...
# test.imp$Pclass <- factor(test.imp$Pclass,
#                                levels = c(1,2,3),
#                                labels = c('P1','P2','P3'))
test.imp$child <- 0
...
test.imp$child <- factor(test.imp$child,
                               levels = c(0,1),
                               labels = c('ChldNo','ChldYes'))

test.imp$Cabin <- NULL

test.imp$Fare[is.na(test.imp$Fare)] <- mean(test.imp$Fare[test.imp$Age>60])
test.imp$FareLog <- log(test.imp$Fare)
test.imp$Fare <- NULL
...
output <- write.csv(
    x = data.frame(
        PassengerId = PID,
        Survived = boostPred %>% as.numeric() - 1
    ),
    file = 'mar19.csv',
    row.names = F
)
