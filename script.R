...
train <- read.csv("~/Documents/Data Science/Kaggle/Titanic/train.csv",stringsAsFactors=FALSE)
...
submit <- data.frame(PassengerId = test$PassengerId, Survived = test$Survived)
write.csv(submit, file = "femalessurvive_pclass.csv", row.names = FALSE)
...
Prediction <- predict(fit, test, type = "class")
submit <- data.frame(PassengerId = test$PassengerId, Survived = Prediction)
write.csv(submit, file = "my2nddtree.csv", row.names = FALSE)
